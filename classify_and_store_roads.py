...
class 道路分类器:
    ...
    def 分类要素(self, feature: Dict[str, Any]) -> str:
        ...
        是否为道路 = self.判断是否为道路(properties)
        是否为服务区内部道路 = self.判断是否为服务区内部道路(properties)
        
        # 8个分类：有/无名称 × 是/非道路 × 是/非服务区内部道路
        if 是否有名称 and 是否为道路 and 是否为服务区内部道路:
            return "有名称_是道路_服务区内部"
        elif 是否有名称 and 是否为道路 and not 是否为服务区内部道路:
            return "有名称_是道路_非服务区内部"
        elif 是否有名称 and not 是否为道路 and 是否为服务区内部道路:
            return "有名称_非道路_服务区内部"
        elif 是否有名称 and not 是否为道路 and not 是否为服务区内部道路:
            return "有名称_非道路_非服务区内部"
        elif not 是否有名称 and 是否为道路 and 是否为服务区内部道路:
            return "无名称_是道路_服务区内部"
        elif not 是否有名称 and 是否为道路 and not 是否为服务区内部道路:
            return "无名称_是道路_非服务区内部"
        elif not 是否有名称 and not 是否为道路 and 是否为服务区内部道路:
            return "无名称_非道路_服务区内部"
        else:
            return "无名称_非道路_非服务区内部"
        ...
        try:
            ...
            for 分类名称, 要素列表 in 分类数据.items():
                if 要素列表:
                    ...
                    if "是道路" in 分类名称:
                        道路数据.extend(要素列表)
                        logger.info(f"道路分类 '{分类名称}': {len(要素列表)} 个要素")
                    else:
                        非道路数据.extend(要素列表)
                        logger.info(f"非道路分类 '{分类名称}': {len(要素列表)} 个要素")
            
            # 增量存储道路数据
            if 道路数据:
                # 查找现有道路数据文档
                现有道路文档 = self.db[道路集合名称].find_one({"数据类型": "道路数据"})
                
                if 现有道路文档:
                    # 更新现有文档
                    现有要素 = 现有道路文档.get("features", [])
                    现有要素.extend(道路数据)
                    
                    self.db[道路集合名称].update_one(
                        {"数据类型": "道路数据"},
                        {
                            "$set": {
                                "要素数量": len(现有要素),
                    ...
                    logger.info(f"更新道路数据: 新增 {len(道路数据)} 个要素, 总计 {len(现有要素)} 个要素")
                else:
                    # 创建新文档
                    道路文档 = {
                    ...
                    result = self.db[道路集合名称].insert_one(道路文档)
                    logger.info(f"创建道路数据: {len(道路数据)} 个要素, 文档ID: {result.inserted_id}")
            
            # 增量存储非道路数据
            if 非道路数据:
                # 查找现有非道路数据文档
                现有非道路文档 = self.db[非道路集合名称].find_one({"数据类型": "非道路数据"})
                
                if 现有非道路文档:
                    # 更新现有文档
                    现有要素 = 现有非道路文档.get("features", [])
                    现有要素.extend(非道路数据)
                    
                    self.db[非道路集合名称].update_one(
                        {"数据类型": "非道路数据"},
                        {
                            "$set": {
                                "要素数量": len(现有要素),
                    ...
                    logger.info(f"更新非道路数据: 新增 {len(非道路数据)} 个要素, 总计 {len(现有要素)} 个要素")
                else:
                    # 创建新文档
                    非道路文档 = {
                        "type": "FeatureCollection",
                    ...
                    result = self.db[非道路集合名称].insert_one(非道路文档)
                    logger.info(f"创建非道路数据: {len(非道路数据)} 个要素, 文档ID: {result.inserted_id}")
    ...
    def 打印统计结果(self, 统计信息: Dict[str, int]):
        ...
        print("\n分类统计:")
        print(f"  有名称_是道路_服务区内部: {统计信息['有名称_是道路_服务区内部']:,} 个")
        print(f"  有名称_是道路_非服务区内部: {统计信息['有名称_是道路_非服务区内部']:,} 个")
        print(f"  有名称_非道路_服务区内部: {统计信息['有名称_非道路_服务区内部']:,} 个")
        print(f"  有名称_非道路_非服务区内部: {统计信息['有名称_非道路_非服务区内部']:,} 个")
        print(f"  无名称_是道路_服务区内部: {统计信息['无名称_是道路_服务区内部']:,} 个")
        print(f"  无名称_是道路_非服务区内部: {统计信息['无名称_是道路_非服务区内部']:,} 个")
        print(f"  无名称_非道路_服务区内部: {统计信息['无名称_非道路_服务区内部']:,} 个")
        print(f"  无名称_非道路_非服务区内部: {统计信息['无名称_非道路_非服务区内部']:,} 个")
    ...
    def 查询分类数据(self, 数据类型: str = None):
        """查询分类数据"""
        try:
            if 数据类型 == "道路" or 数据类型 == "道路数据":
                集合名称 = "道路数据"
            elif 数据类型 == "非道路" or 数据类型 == "非道路数据":
                集合名称 = "非道路数据"
            else:
                # 查询所有数据
                道路结果 = list(self.db["道路数据"].find())
                非道路结果 = list(self.db["非道路数据"].find())
                
                print(f"\n查询所有数据:")
                for 文档 in 道路结果:
                    print(f"数据类型: {文档['数据类型']}")
                    print(f"要素数量: {文档['要素数量']}")
                    print(f"创建时间: {文档['创建时间']}")
                    print("-" * 40)
            ...
            结果 = list(self.db[集合名称].find(查询条件))
