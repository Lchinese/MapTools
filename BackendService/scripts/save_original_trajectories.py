...
def save_original_trajectories_to_mongodb(
    ...
    source_collection_name: str = "gps_points",
    target_db_name: str = "MapTools",
    target_collection_name: str = "original_trajectories",
    batch_size: int = 100,
    match_to_roads: bool = False
):
    """
    将原始轨迹数据保存到MongoDB的original_trajectories集合
    
    Args:
        source_db_name: 源数据库名称
        source_collection_name: 源集合名称（gps_points）
        target_db_name: 目标数据库名称
        target_collection_name: 目标集合名称（original_trajectories）
        batch_size: 批处理大小
        match_to_roads: 是否进行道路匹配
    """
    ...
    try:
        ...
        # 2. 连接到目标MongoDB
        target_collection, target_client = connect_to_mongodb(target_db_name, target_collection_name)
        if target_collection is None:
            logger.error("无法连接到目标MongoDB")
            return False
        ...
        # 4. 获取所有车牌号（分页查询）
        all_plates, total_plates, total_pages = fetch_plate_numbers(
            page=1, 
            page_size=20000,  # 获取所有车牌号
            db_name=source_db_name,
            collection_name=source_collection_name
        )
        
        logger.info(f"找到 {total_plates} 个车牌号，开始处理...")
        ...
        for plate_number in all_plates:
            try:
                ...
                if processed_plates % 100 == 0:
                    ...
                    logger.info(f"[{elapsed}] 进度: {processed_plates}/{total_plates} | 已保存: {saved_count} | 跳过: {skipped_count}")
        ...
        # 6. 验证数据
        total_saved = target_collection.count_documents({})
        logger.info(f"验证完成，MongoDB中共有 {total_saved} 条原始轨迹数据")
        
        # 7. 保存统计信息
        stats_doc = {
            "_id": "metadata",
            "total_trajectories": total_saved,
        ...
        logger.info("✅ 轨迹数据保存完成！")
        logger.info(f"📊 总计: {total_saved} 条轨迹 | 处理: {processed_plates} 个车牌 | 跳过: {skipped_count} 个 | 耗时: {total_time}")
